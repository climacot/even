...
export function CorrelationMatrix({ matrix }: CorrelationMatrixProps) {
  ...
  const generateMockData = (correlationValue: number) => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const y =
        x * correlationValue + (Math.random() - 0.5) * baseNoise * 5 + 2.5;
      data.push({
        ...
        <div className="space-y-4">
                ...
                <thead>
                  <tr>
                    ...
                    {matrix.variables.columns.map((col, idx) => (
                      <th
                        key={idx}
                        className="border border-border p-3 bg-muted min-w-[100px]"
                      >
                          ...
                          <TooltipProvider key={colIndex}>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <td
                                  className={`border border-border p-3 text-center cursor-pointer transition-opacity ${getCorrelationHoverColor(
                                    value
                                  )}`}
                                  style={colorStyle}
                                  onClick={() =>
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm text-popover-foreground">
                                      {getCorrelationLegend(value as number)}
                                    </span>
                                  </div>
          ...
          {(matrix.hypothesis || matrix.conclusion) && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {matrix.hypothesis && (
                <Collapsible
                  open={isHypothesisOpen}
                  onOpenChange={setIsHypothesisOpen}
                >
        ...
        <SheetContent className="w-full sm:max-w-3xl overflow-y-auto px-8">
          {selectedCell &&
            (() => {
              ...
              return (
                <>
                  <SheetHeader className="px-0[py-44 pr-[0pr-00px] pl-0">
                    <SheetTitle>Datos de Correlación</SheetTitle>
                    <SheetDescription>
                      Correlación entre <strong>{selectedCell.rowName}</strong>{" "}
                      y <strong>{selectedCell.colName}</strong>
                    </SheetDescription>
                  </SheetHeader>

                  <div className="mt-6 space-y-6">
                    <div className="flex items-center justify-between bg-[rgb(243,244,246)] rounded-lg p-4">
                      <div>
                        <p className="text-muted-foreground text-sm">
                          Coeficiente de Pearson
                        </p>
                        <p className="text-foreground">
                          r = {selectedCell.value.toFixed(3)}
                        </p>
                      </div>
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          ></div>
                          <p className="text-foreground">
                            {getCorrelationStrength(selectedCell.value)}
                          </p>
                        </div>
                      </div>
                      <div>
                        ...
                        <p className="text-foreground">
                          {selectedCell.value > 0 ? "Positiva" : "Negativa"}
                        </p>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="border border-border rounded-lg p-4 bg-card">
                                  ...
                                  return (
                                    <div className="bg-popover border border-border rounded-lg p-3 shadow-lg">
                                      <p className="text-popover-foreground text-sm">
                                        <strong>{selectedCell.rowName}:</strong>{" "}
                                        {payload[0].value}
                                      </p>
                                      <p className="text-popover-foreground text-sm">
                                        <strong>{selectedCell.colName}:</strong>{" "}
                                        {payload[1].value}
                                      </p>
                                    </div>
                                  );
                        ...
                        <p className="text-muted-foreground text-xs text-center mt-2">
                          La línea de tendencia muestra la relación{" "}
                          {selectedCell.value > 0 ? "positiva" : "negativa"}{" "}
                          entre las variables
                        </p>
                      </div>
                    ...
                    <div>
                      <h4 className="text-foreground mb-3">
                        Datos observados (muestra de 30 sesiones)
                      </h4>
