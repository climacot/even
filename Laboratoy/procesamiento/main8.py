...
plt.title('Distribución de JSON-LD según la Calificación de Pertinencia')
plt.xlabel('Calificación (3-5)')
plt.ylabel('Cantidad de JSON-LD')
...
if p_value < 0.05:
    ...
    if coef > 0:
        print(">> Dirección: La correlación es POSITIVA (más JSON-LD tiende a mejor calificación).")
    else:
        print(">> Dirección: La correlación es NEGATIVA (más JSON-LD tiende a peor calificación).")
else:
...
grupos = [df[df['calificacion_pertinencia'] == i]['num_jsonld'] for i in range(3, 6)]
...
if p_val_kruskal < 0.05:
    print(">> Conclusión: La cantidad de JSON-LD varía significativamente entre los diferentes grupos de calificación.")
else:
    print(">> Conclusión: No hay diferencias significativas en la cantidad de JSON-LD entre las calificaciones.")